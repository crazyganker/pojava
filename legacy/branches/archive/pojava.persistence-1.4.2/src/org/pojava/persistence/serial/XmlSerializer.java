        ...
        if (ReflectionTool.isBasic(type) || type.equals(Object.class)) {
            ...
            for (Iterator it = getters.keySet().iterator(); it.hasNext();) {
                ...
                if (!config.isOmission(type, property)) {
                    try {
                        Object obj = ((Method) getters.get(key)).invoke(pojo, null);
                        walk(obj);
    ...
    private String snippetFromPojo(Object pojo, String name, String attribs, int depth,
        ...
        try {
            ...
            for (Iterator it = getters.keySet().iterator(); it.hasNext();) {
                ...
                if (!config.isOmission(type, property)) {
                    Object innerPojo = ((Method) getters.get(property)).invoke(pojo, null);
                    /*
                     * String renamed = config.renamedJava(type, property); if (renamed != null)
                     * { property = renamed; }
                     */
