...
public class ReflectionTool {
    ...
    public static void setNestedValue(String path, Object parent, Object child)
        ...
        try {
            ...
            for (int p = 0; p < pathPart.length - 1; p++) {
                ...
                try {
                    if (pathPart[p].indexOf('[') >= 0) {
                        ...
                        method = innerObjectClass.getMethod(getter(pathPart[p]), null);
                    }
                } catch (NoSuchMethodException ex) {
                    method = innerObject.getClass().getMethod(pathPart[p], null);
                }
                if (offset == null) {
                    ...
                    if (innerObjectClass.isArray()) {
                        ...
                        // iterated.
                        innerObject = ((Map) innerObject).values().toArray()[offset.intValue() - 1];
                    } else {
            ...
            try {
                ...
                method = innerObject.getClass().getMethod(pathPart[pathPart.length - 1], null);
            }
     ...
     */
    public static Accessors accessors(Class type, Map getterNames, Map setterNames)
            throws NoSuchMethodException {
        Method[] allMethods = type.getMethods();
        Accessors accessors = new Accessors();
        for (int i = 0; i < allMethods.length; i++) {
        ...
        for (int p = 0; p < path.length; p++) {
            if (path[p].indexOf('[') >= 0) {
                method = innerClass.getMethod(
                        getter(path[p].substring(0, path[p].indexOf('['))), null);
            } else {
                try {
                    method = innerClass.getMethod(getter(path[p]), null);
                } catch (NoSuchMethodException ex) {
                    method = innerClass.getMethod("is" + StringTool.capitalize(path[p]), null);
                }
            }
            methods[p] = method;
            innerClass = method.getReturnType();
            ...
            for (int p = 0; p < path.length; p++) {
                ...
                try {
                    if (path[p].indexOf('[') >= 0) {
                        ...
                        method = innerObjectClass.getMethod(getter(path[p]), null);
                    }
                } catch (NoSuchMethodException ex) {
     ...
     */
    public static void populateFromMap(Object obj, Map properties, Map setters)
            throws IllegalAccessException, InvocationTargetException {
        if (obj == null || setters == null || setters.isEmpty()) {
            return;
        }
        for (Iterator it = setters.keySet().iterator(); it.hasNext();) {
