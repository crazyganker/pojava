...
/**
 * DaoTool provides a consistent assortment of database operations for your DAO objects to
 * provide. It takes much of the grunt-work out of the typical CRUD operations often copy-pasted
 * and altered from one DAO to another.
 * 
 * @author John Pile
 * 
 */
public class DaoTool {
     ...
     * @param action
     */
    private static final void validateParams(TableMap map, SqlQuery query, String action) {
        if (query == null) {
     ...
     * @param action
     */
    private static final void validateParams(TableMap map, Object obj, String action) {
        if (obj == null) {
            StringBuffer msg = new StringBuffer();
            msg.append("Cannot perform ");
            msg.append(action);
    ...
    /**
     * Perform a single insert, throwing an exception if the attempted insert fails. This will
     * throw an SQLException if an existing record matches the primary key defined in the
     * TableMap.
     * 
     * @param conn
     * @param map
    ...
    /**
     * Perform a batch insert, throwing an exception if the attempted insert fails. Pair this
     * with processByQuery for fast high-volume transfers.
     * 
     * @param conn
     *            Open connection to a databases
     * @param map
     ...
     */
    public static final int[] batchInsert(Connection conn, TableMap map, List list) {
    ...
    /**
     * Update an existing object in the table according to the primary key, and insert a new
     ...
     */
    public static final int updateInsert(Connection conn, TableMap map, Object obj) {
    ...
    /**
     * Insert a new record into a table, but only if that record is not already present
     * according to its primary key.
     * 
     * @param conn
     * @param map
     * @param obj
     ...
     */
    public static final int passiveInsert(Connection conn, TableMap map, Object obj) {
    ...
    /**
     * Update an existing record in a table. The record is selected by its key fields and its
     * non-key fields are updated. This does not throw an exception if the record does not
     * already exist.
     * 
     * @param conn
     * @param map
     ...
     */
    public static final List listByQuery(Connection conn, TableMap map, SqlQuery query) {
    ...
    /**
     * Process a list of objects matching the query using the user-defined class extending
     * Processor. Each row is packaged into a bean according to the TableMap, and then
     * individually sent to the "process" method of the provided Processor object. In this way,
     * even very large query results can be processed with very low memory consumption within
     ...
     */
    public static final int processByQuery(Connection conn, TableMap map, SqlQuery query,
            Processor objProcessor) {
        validateParams(map, query, "processByQuery");
        try {
            ResultSetToProcessor processor = new ResultSetToProcessor(map, objProcessor);
            return SqlTool.executeQuery(query.generatePreparedSql(map.sqlSelect()), conn,
                    processor);
        } catch (SQLException ex) {
            throw new PersistenceException(ex.getMessage(), ex);
        }
     ...
     */
    public static final int countByQuery(Connection conn, TableMap map, SqlQuery query) {
        try {
            if (query == null) {
                StringBuffer msg = new StringBuffer();
     ...
     * @return number of rows deleted
     */
    public static final int deleteByQuery(Connection conn, TableMap map, SqlQuery query) {
        try {
            if (query == null) {
                StringBuffer msg = new StringBuffer();
