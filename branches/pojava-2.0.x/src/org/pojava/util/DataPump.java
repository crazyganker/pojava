...
/**
 * The DataPump class was developed primarily for supporting
 * Runtime.getRuntime().exec(...), where you may need to gather from stdout and
 * stderr simultaneously. Handling results using an additional thread or two
 * could prevent an app from locking up while waiting for you to read from one
 * of those two pipes. If you exec a single-threaded app that completely fills
 * one 4k pipe while you're waiting for data on the other pipe, it can block
 * and may never get around to either writing to or closing the pipe you're
 * reading-- a classic deadlock preventable by threading.
 * 
 * @author John Pile
 */
public class DataPump extends Thread {
	 ...
	 *             if read/write fails
	 */
	public static void pump(final InputStream in, final OutputStream out)
			throws IOException {
	 ...
	 *             if read/write fails
	 */
	public static void pump(final InputStream in, final StringBuffer buffer)
			throws IOException {
