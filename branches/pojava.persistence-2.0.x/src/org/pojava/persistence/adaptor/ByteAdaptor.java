...
public class ByteAdaptor extends BindingAdaptor<Byte, Byte> {
	...
	public Binding<Byte> inbound(Binding<Byte> inBinding) {
		if (inBinding == null || inBinding.getObj() == null || Byte.class.equals(inBinding.getType())) {
			return inBinding;
		}
		Binding<Byte> outBinding = new Binding<Byte>(Byte.class, null);
		/*
		 * When DB value is a smallint, JDBC will receive it as type Integer.
		 * With the DAO layer seeing only Integer to Byte, there's a danger of
		 * overflow, but we trust the developer had valid reason to choose Byte
		 * over Integer in the bean.
		 */
		outBinding.setValue(Byte.valueOf((String) inBinding.getObj()
					.toString()));
		return outBinding;
	}
